...
export function DetailPage() {
  ...
  async function removeTrade(trade: TradeResult) { if (!confirm(`删除 ${formatEt(trade.executedAt)} 的这笔交易？之后的 FIFO 盈亏会重新计算。`)) return; try { await window.stockEarn.trades.delete(trade.id); await load(); } catch (reason) { setNotice(errorMessage(reason)); } }
