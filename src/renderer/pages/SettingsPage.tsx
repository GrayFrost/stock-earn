...
export function SettingsPage({ onSettingsChange }: { onSettingsChange: (settings: AppSettings) => void }) {
  ...
  async function restoreBackup() { if (!confirm('恢复备份会替换当前账本。应用会先自动保存一份恢复前备份，是否继续？')) return; setBusy('restore'); try { const result = await window.stockEarn.backup.restore(); if (result.restored) { clearStoredFontSize(); window.location.reload(); } } catch (reason) { setNotice(errorMessage(reason)); setBusy(''); } }
  ...
  return <div className="page settings-page"><header className="page-header"><div><p className="eyebrow">LEDGER SETTINGS</p><h1>设置</h1><p>管理账本起点、交易平台和本地数据。</p></div></header>{notice && <div className="notice-bar">{notice}</div>}
    <div className="settings-grid">
      <section className="settings-card"><div className="settings-card-title"><Landmark /><div><h2>账本规则</h2><p>起始日不能晚于已有交易。</p></div></div><div className="form-stack"><label>入市起始日<Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} /></label><Button variant="primary" className="align-self" onClick={saveGeneral} disabled={busy === 'general'}><Save size={16} />{busy === 'general' ? '保存中…' : '保存账本规则'}</Button></div></section>
      ...
      <section className="settings-card span-two"><div className="settings-card-title"><DatabaseBackup /><div><h2>本地备份</h2><p>导出交易、行情缓存和设置；API Key 不包含在内。</p></div></div><div className="backup-actions"><Button variant="secondary" onClick={exportBackup} disabled={busy === 'backup'}><DatabaseBackup size={16} />{busy === 'backup' ? '导出中…' : '导出备份'}</Button><Button variant="danger" onClick={restoreBackup} disabled={busy === 'restore'}><RotateCcw size={16} />{busy === 'restore' ? '恢复中…' : '从备份恢复'}</Button></div></section>
    </div>
  </div>;
}
