...
export function PositionsTable({ data, hasInstruments, onAddStock, onTrade, onOpen, onArchive }: {
  ...
  return <div className="table-scroll"><table className="positions-table"><thead>{table.getHeaderGroups().map((group) => <tr key={group.id}>{group.headers.map((header) => <th key={header.id} className={header.column.id === 'instrument' ? 'symbol-col' : ''}>{header.isPlaceholder ? null : header.column.getCanSort() ? <button className="sort-header" onClick={header.column.getToggleSortingHandler()}>{flexRender(header.column.columnDef.header, header.getContext())}{header.column.getIsSorted() === 'asc' ? <ArrowUp size={12} /> : header.column.getIsSorted() === 'desc' ? <ArrowDown size={12} /> : <ArrowUpDown size={12} />}</button> : flexRender(header.column.columnDef.header, header.getContext())}</th>)}</tr>)}</thead><tbody>
